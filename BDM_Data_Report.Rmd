...
```{r D4A Industries with S3, fig.width=10, fig.height=7}
S3_data <- data_frame(S3 = 1:10, 
                      Variable = c("1/Sole proprietorship","2-10","11-25","26-50","51-75","76-100","101-499","500-1000","1000+","Refuse"))
...
D4A_plot_data <- BDM_Final_Clean %>% 
  ...
  group_by(Size) %>% 
  dplyr::mutate(Percent =round(Count/sum(Count),3)) %>% 
  ungroup() %>% 
...
D4A_plot_data1 <- D4A_plot_data %>% 
    mutate(under_100 = Size %in% c("2-10", "11-25", "26-50", "51-75", "76-100")) %>% 
    count(Industry, under_100, name = "company_count") %>% 
    group_by(Industry) %>% 
    mutate(percent = company_count / sum(company_count)) %>% 
    ungroup() %>% 
    pivot_wider(names_from = under_100,
                values_from = percent,
                values_fill = 0) %>% 
    mutate(Industry = fct_reorder(Industry, `TRUE`)) %>% 
...
```{r D4A ggplotly graph, include=FALSE}
ggplotly(
  D4A_plot_data %>%
    ggplot(aes(x = percent,
               ...
               text = paste(
                 ...
                 "<br><b>&#8226; Count: </b>",Count,
                 "<br><b>&#8226; Within Company Size Percentage: </b>",percent_less_100
               ))) +
    geom_bar(stat = "identity") +
    geom_col() +
    ggtitle("Company Size Across Primary Industries") +
    ylab("Industries") +
    xlab("Response of the Business ") +
...
```{r D4A plot using plotly-a,fig.width=10, fig.height=7}
library(plotly)
fig <- plot_ly(
  data = D4A_plot_data,
  x = ~Count,
  ...
  hovertemplate = ~paste(
    '</br><b> %{text}</b><br>',
    '</br> <b>Count:</b>',Count,
    '</br> <b>Within company size:</b>',Percent)
)
fig <- fig %>% 
  layout(barmode = "stack",
         ...
         legend = list(title = list(text = '<b> Primary Industries </b>'))
  )
fig
```
...
```{r D4A plot using plotly-b,fig.width=10, fig.height=7, include=FALSE}
plot_ly(
  data = D4A_plot_data,
  x = ~Industry,
  y = ~Count,
  ...
  hovertemplate = ~paste(
    '</br><b> %{text}</b><br>',
    '</br> <b>Count:</b>',Count,
    '</br> <b>Within company size:</b>',Percent_size
  )
) %>% 
  layout(barmode = "stack", legend = list("2-10","11-25","26-50","51-75","76-100","101-499","500-1000","1000+"))
```




```{r Q3 plot over time fro each industry-b, fig.width=10}
library(dplyr)
library(ggplot2)
...
Q12_count_all_ind <- Q12_count_all_ind %>% 
  ...
  replace(is.na(.), 0) %>% 
  mutate(Industry = as.factor(Industry))
#Q12_count_all_ind <- inner_join(labels,Q12_count_all_ind, by="Industry") 
...
ggplot(Q16_sum,aes(x= "",y=count, fill=Question)) +
  geom_bar(width = 1, stat = "identity", color="white", alpha=0.8) +
  coord_polar("y",start = 0) +
  ggtitle("Top-5 Services Recommended for Delaware JobLink") +
  geom_text(aes(label = labels),position = position_stack(vjust = 0.5), size=4.2, color = "white") +
...
Q17_tr_label <- data.frame("Q17_list" = c(1:13),"Variable" = Q17_labels,"training"=c("Q17A High schools",
"Q17B Career and Technical Education programs",
"Q17C Libraries",
"Q17D Delaware 2-year public colleges",
"Q17E Delaware 4-year public colleges",
"Q17F Delaware 4-year Private colleges",
"Q17G Out of state 2-year public colleges",
"Q17H Out of state 4-year public colleges",
"Q17I Out of state 4-year Private colleges",
"Q17J Labor unions",
"Q17K Local workforce development board(s)",
"Q17L Non-profit/private training providers",
"Q17M Apprenticeships"))
...
```{r Q17 line plot, include=FALSE}
# ggplot(data = Q17_count, aes(x=Q17_value, y=Total_Count, group=1)) +
#   geom_line() + geom_point() +
#   theme(axis.text.x = element_text(angle = 90)) +
#   theme_bw() +
#   ggtitle("Distribution of Very and Somewhat Important Responses by Industry") +
...
ggplot(data = Q17_sum, aes(x=training, y=Count, group=1)) +
  geom_line() + geom_point() +
  theme(axis.text.x = element_text(angle = 90)) 
```

```{r Q17-a,fig1, fig.height = 5, fig.width = 7}
ggplotly( 
Q17_sum %>% 
...
```{r Q17-b, fig2, fig.height = 5, fig.width = 8}
ggplotly( 
  Q17_count_growth_ind %>% 
    ggplot(aes(x=Variable, 
               y=value,
               fill=names,
               text = paste(
                 "<b>&#8226; Industry: </b>",names,
                 "<br><b>&#8226; Training: </b>",training,
                 "<br><b>&#8226; Count: </b>",value
               ))) +
    geom_bar(stat = "identity") +
    theme_classic() +
    ...
    xlab("Applicant’s Education Level (Training)") +
    ylab("Very/Somewhat Important Responses (Count)") +
    guides(fill=guide_legend(title="Growth Industries")) +
...
```{r Q18 what training offered? PCA analysis}
...
```{r Q20 PCA analysis}
# data <- BDM_Final_Clean[25:34]
# pca_res <- prcomp(data, scale. = TRUE)
# autoplot(pca_res,data = BDM_Final_Clean, colour="D4A")
# summary(pca_res)
```
...
### How many positions you’re actively trying to fill?  - Averaged


* The considirations:
  + We need to look at the Company sizes across industries: based on averaged size per industry, we can make assumptions on growth. Because the number of employees the company will hire is highly correlated to the company size. 
...
Q30_count_all_ind <- Q30_count_all_ind %>% 
  ...
  replace(is.na(.), 0) %>% 
  mutate(Industry = as.factor(Industry))
#Q30_count_all_ind <- inner_join(labels,Q30_count_all_ind, by="Industry") 
...
```{r Q30 growth industries plot}
Q30_count_growth_ind$Answer <- factor(Q30_count_growth_ind$Answer,levels = c("Yes", "No", "Not Sure"))

ggplotly(
  Q30_count_growth_ind %>%
    mutate(Percent_yes = if_else(Answer=="Yes",Percent,NA_real_)) %>%
    fill(Percent_yes) %>%
    mutate(`Growth Industry`=fct_reorder(`Growth Industry`, Percent_yes)) %>% 
    ggplot(aes(x= Percent,
               y=`Growth Industry`,
               fill=Answer,
               text=paste(
                 "<b>&#8226; Growth industry: </b>",`Growth Industry`,
                 "<br><b> &#8226; Count: </b>",Count,
                 "<br><b> &#8226; Response within industry %: </b>",Percent*100,
                 "<br><b> &#8226; Response within answer %: </b>",Ans*100,
                 "<br><b> &#x2022; Total within industry: </b>",G_Total),
               group=`Growth Industry`
    )) +
...
### Do you have positions not requiring a college degree?
...
Q34_count_all_ind <- Q34_count_all_ind %>% 
  ...
  replace(is.na(.), 0) %>% 
  mutate(Industry = as.factor(Industry))
#Q34_count_all_ind <- inner_join(labels,Q34_count_all_ind, by="Industry") 
...
Q34_count_growth_ind$Answer <- factor(Q34_count_growth_ind$Answer,
                                levels=c("Yes","No","Not Sure"))


plot_ly(
  data = Q34_count_growth_ind,
  x = ~Answer,
  y = ~Count,
  color = ~`Growth Industry`,
  type = "bar",
...
```{r Q35 plot}
ggplotly(
  Q35_count_growth_ind %>% 
ggplot(aes(x=fct_relevel(Answer, "Yes"),
          y=Count,
        fill=`Growth Industry`,
        text=paste(
         "<b>&#8226; Growth Industry: </b>",`Growth Industry`,
         "<br><b>&#8226; Count: </b>",Count,
         "<br><b> &#8226; Within industry: </b>",Percent,
         "<br><b> &#8226; Within answer: </b>",Ans,
         "<br><b> &#x2022; Total within Industry: </b>",G_Total),
        group=`Growth Industry`
       )) +
  geom_bar(stat = "identity") +
  ggtitle("Do You Accept Individuals Coming out of the Criminal Justice System by Industry?") +
  xlab("Response of the Business") +
  ylab("Response of the Business (Count)") +
    # #scale_fill_brewer(palette="Set1") +
    # #scale_x_discrete(labels= q35_label) + 
    # #theme(axis.text.x = element_text(angle  = 90)) +
  theme(plot.title = element_text(size = 11, face = "bold")) +
  theme(axis.title.y = element_text(size = 11)) +
  theme(axis.text.x = element_text(size = 8)) +
...
Q36_count_all_ind <- Q36_count_all_ind %>% 
  ...
  replace(is.na(.), 0) %>% 
  mutate(Industry = as.factor(Industry))
#Q36_count_all_ind <- inner_join(labels,Q36_count_all_ind, by="Industry") 
...
ggplot(aes(x=fct_relevel(Answer, "Yes"),
  ...
  geom_bar(stat = "identity") +
  ggtitle("How ready are new applicants?") +
  xlab("Response of the Business") +
  ylab("Response of the Business (Count)") +
    # #scale_fill_brewer(palette="Set1") +
    # #theme(axis.text.x = element_text(angle  = 90)) +
...
plot_ly(
  ...
  layout(barmode = "stack",
         ...
         xaxis = list(title = list(text='<b> Responses</b>'), tickangle = 330),
...
## How many mission critical positions will be lost due to retirement {.tabset}


### How many mission critical positions will be lost due to retirement in the next year (Q37) and next five years (Q38)?
...
fig <- fig %>% 
  layout(title = list(text = '<b> How many mission critical positions will be lost due to retirement? - Averaged </b>'),
